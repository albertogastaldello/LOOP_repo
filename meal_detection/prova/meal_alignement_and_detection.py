...
timestamp = pd.to_datetime(data['t'], format='mixed')

unique_days = timestamp.dt.normalize().unique() # get unique days

# get data from dataframe
CGM	 = data['glucose'].to_numpy()
Carbohydrates = data['cho'].to_numpy()
...
day = 2
#for k in range(0,len(unique_days)-3,1):
for k in range(day,day+1,1):
    ...
    gprime =  np.clip(signal.savgol_filter(CGM[idx], window_length=20, polyorder=6, deriv=1, delta = 5),0, float('inf'))
    g2prime =  np.clip(signal.savgol_filter(gprime, window_length=20, polyorder=6, deriv=1, delta = 5),0, float('inf'))
    ...
    I1min = 9;  # low bound indices of the S1 search area
    I1max = 11;  # upper bound indices of the S1 search area
    I2min = 4;  # low bound indices of the S2 search area
    ...
    Pm = 0.0175 # height (mg2/dL2/min3)  #(NEW VALUE)
    Pp = 0.0275 #prominence (mg2/dL2/min3) #(NEW VALUE)
    separation  = 45 #(min)
