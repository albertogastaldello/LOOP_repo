function insulinDataAnalysis()

    % evaluate and visualize IOB availability across exercise sessions
    
    exercise = load("exerciseTableForBN.mat").exerciseTableForBN;
    
    nSessions = height(exercise);
    IOB_vector = zeros(nSessions,1);
    
    for i=1:nSessions
        curr_IOB = exercise.insulinData{i,1}.IOB;
        IOB_vector(i,1) = curr_IOB;
    end
    
    IOB_nan = length(find(isnan(IOB_vector)));
